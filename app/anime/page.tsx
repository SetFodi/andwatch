...
// Helper function with explicit object access
function getSearchParams(params: { [key: string]: string | string[] | undefined } | undefined = {}) {
  const safeParams = params as Record<string, string | string[] | undefined>;
  
  const genre = safeParams["genre"];
  const genreId = genre && typeof genre === "string" ? parseInt(genre) : null;

  const q = safeParams["q"];
  const searchQuery = q && typeof q === "string" ? q : null;

  const pageParam = safeParams["page"];
  const page = pageParam && typeof pageParam === "string" 
    ? Math.max(1, parseInt(pageParam)) || 1 
    : 1;

  return { genreId, searchQuery, page };
}
...
export default async function AnimeList({
  ...
  const { genreId, searchQuery, page } = getSearchParams(searchParams);
  ...
  try {
    ...
    animeData = { data: [], pagination: { current_page: 1, last_visible_page: 1, has_next_page: false } };
  }

  const animeList = animeData?.data || [];
  console.log("Raw Anime Data:", animeData); // Log raw data for debugging
  console.log("Anime List Data:", animeList.map((anime: any) => ({
    title: anime.title,
    mal_id: anime.mal_id, // Ensure mal_id is present
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <h1 className="text-4xl font-bold">
          {searchQuery
            ? `Search: ${searchQuery}`
            : genreId
            ? `${animeGenres.find((g) => g.id === genreId)?.name || "Genre"} Anime`
            : "Top Anime"}
        </h1>
          ...
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {animeList.map((anime: any, index: number) => {
                // Fallback key if mal_id is missing
                const key = anime.mal_id ?? `anime-${index}`;
                return (
                  <Link href={`/anime/${anime.mal_id}`} key={key} className="block">
                    <div className="bg-gray-800 rounded-lg overflow-hidden shadow-lg hover:shadow-xl transition transform hover:scale-105 h-full flex flex-col">
                        ...
                        {(() => {
                          ...
                          if (imageUrl && typeof imageUrl === "string" && imageUrl.trim() !== "") {
                            try {
                              ...
                              console.error(`Image render error for ${anime.title || "unknown"}:`, error);
                              return (
                      ...
                      <div className="p-4 flex-grow flex flex-col">
                        <h2 className="text-xl font-semibold line-clamp-1">{anime.title || "Untitled"}</h2>
                        <p className="text-sm text-gray-400 line-clamp-2 mt-1">{anime.synopsis || "No synopsis available"}</p>

                        <div className="mt-3 flex flex-wrap gap-1">
                          {anime.genres?.slice(0, 3).map((genre: any) => (
                            <span key={genre.mal_id ?? `${key}-genre-${genre.name}`} className="px-2 py-1 bg-gray-700 rounded text-xs">
                              {genre.name}
                            </span>
                          ))}
