...
export default function MediaCard({ item }: { item: MediaItem }) {
  // Determine theme color based on media type
  const themeColor = item.type === "anime" ? "indigo" : "red";
...
const statusBadgeStyle = () => {
    switch (item.status) {
      ...
      case "plan_to_watch":  // FIX: Use database value
  ...
  return (
    <Link href={item.url} className="block h-full">
      <motion.div 
        className="flex flex-col h-full bg-gray-900/50 backdrop-blur-sm rounded-xl overflow-hidden border border-gray-800/40 hover:border-gray-700/80 shadow-lg hover:shadow-xl transition-all duration-300 group"
  ...
  <div className={`absolute top-2 left-2 px-2 py-0.5 rounded-md text-xs font-medium border ${statusBadgeStyle()}`}>
    {item.status === "plan_to_watch" ? "Planning" : item.status.charAt(0).toUpperCase() + item.status.slice(1)}
  </div>
)}
              {/* Type badge */}
        ...
        <div className="p-4 flex flex-col flex-grow">
          <h3 className="text-white font-medium line-clamp-2 mb-1 group-hover:text-indigo-400 transition-colors duration-300">
            {item.title}
          </h3>
            ...
            {item.score && (
              <>
                ...
                <div className="flex items-center">
                  ...
                  <span className={`text-${themeColor}-300`}>{item.score}</span>
                </div>
              </>
            )}
          </div>
