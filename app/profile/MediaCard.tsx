...
export default function MediaCard({ item }: { item: MediaItem }) {
  ...
  const themeColor = item.type === "anime" ? "indigo" : item.type === "tv" ? "blue" : "red";
    ...
    <Link href={item.url} className="block h-full">
      ...
      >
        ...
        <div className="p-4 flex flex-col flex-grow min-h-[6rem]">
          <h3 className={`text-white font-medium line-clamp-2 mb-1 group-hover:text-${themeColor}-400 transition-colors duration-300 ${isPlaceholder ? 'opacity-60' : ''}`}>
            {item.title}
          </h3>
