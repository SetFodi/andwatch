...
export default function HistoryCard({ item }: { item: HistoryItem }) {
  ...
  // Theme color class based on media type
  ...
  // Determine the activity text based on status and timestamps
  const getActivityText = () => {
    if (item.completedAt && item.status === 'completed') {
      return `Completed ${formatTimestamp(item.completedAt)}`;
    } else if (item.status === 'watching') {
      return `Started watching ${formatTimestamp(item.addedAt)}`;
    } else if (item.status === 'plan_to_watch') {
      return `Added to planning ${formatTimestamp(item.addedAt)}`;
    } else if (item.userRating) {
      return `Rated ${item.userRating}/10 ${formatTimestamp(item.updatedAt)}`;
    } else {
      return `Updated ${formatTimestamp(item.updatedAt || item.addedAt)}`;
    }
  };
  ...
  return (
    <Link href={item.url || "#"} className="block h-full">
      ...
      >
        ...
        {/* Content area with activity timestamp */}
        <div className="p-4 flex flex-col flex-grow">
          ...
          {/* Activity timestamp */}
          <div className="mt-3 text-xs text-amber-400/80 bg-amber-900/20 border border-amber-800/30 rounded-md px-2 py-1">
            <div className="flex items-center">
